// Entry point for WinFuzz's injected server DLL
#include "pch.h"
#include "winfuzz.h"
#include "util.h"
...
// WinFuzz variables
static unsigned long timesRun = 0;
...
// WinFuzz functions
void copyMutableSections()
{
	// Copy sections from executable module
...
#ifdef DEBUG_PRINT
			printf("VA of copied section: %p\n", copy.start);
#endif
			copy.size = ((section[i].Misc.VirtualSize + sizeof(Page) - 1) / sizeof(Page)) * sizeof(Page);
			copy.ptr = (Page*)real_malloc_ptr(copy.size);
	...
	DWORD bytesNeeded = 0;
	if (EnumProcessModules(GetCurrentProcess(), modules, 1024 * sizeof(HMODULE), &bytesNeeded) == 0)
	{
		printf("failed\n.");
		return;
	}
	for (unsigned i = 0; i < trackedDlls.size(); i++)
	{
		int numRecieved = bytesNeeded / sizeof(HMODULE);
		printf("recieved %d process modules\n", numRecieved);
		for (int j = 0; j < numRecieved; j++)
		{
			char fileName[512];
			if (GetModuleFileNameExA(GetCurrentProcess(), modules[j], fileName, 512) == 0)
			{
				printf("failed.\n");
				continue;
			}
			//printf("%s == %s?\n", endOfPath(trackedDlls[i]), endOfPath(fileName));
			lowercase(fileName);
			if (strcmp(endOfPath(trackedDlls[i]), endOfPath(fileName)) == 0)
			{
				printf("Tracking DLL %s\n", fileName);
				MODULEINFO info;
				if (GetModuleInformation(GetCurrentProcess(), modules[j], &info, sizeof(info)) == 0)
				{
				...
				// We've found the target DLL, so add its mutable pages to the list
				char* baseOfDll = (char*)info.lpBaseOfDll;
				ntHeaders = (PIMAGE_NT_HEADERS)((PCHAR)baseOfDll + ((PIMAGE_DOS_HEADER)baseOfDll)->e_lfanew);
				section = IMAGE_FIRST_SECTION(ntHeaders);
				for (ULONG k = 0; k < ntHeaders->FileHeader.NumberOfSections; k++)
				{
					if (section[k].Characteristics & IMAGE_SCN_MEM_WRITE)
					{
						SectionCopy copy;
						copy.start = baseOfDll + section[k].VirtualAddress;
#ifdef DEBUG_PRINT
						printf("VA of copied section: %p\n", copy.start);
#endif
						copy.size = ((section[k].Misc.VirtualSize + sizeof(Page) - 1) / sizeof(Page)) * sizeof(Page);
						copy.ptr = (Page*)real_malloc_ptr(copy.size);
						memcpy(copy.ptr, copy.start, copy.size);
						memcpy(copy.name, section[k].Name, 8);
						copy.name[8] = '\0';
						numPages += (section[k].Misc.VirtualSize + sizeof(Page) - 1) / sizeof(Page);
						sectionCopies.push_back(copy);
						printf("Made a copy of section %s in module %s\n", copy.name, trackedDlls[i]);
					}
				}
			}
		}
	}
	modifiedPages.reserve(numPages);
}

void guardMutableSections()
{
	char* base = (char*)getExeBase();
	PIMAGE_NT_HEADERS ntHeaders = (PIMAGE_NT_HEADERS)((PCHAR)base + ((PIMAGE_DOS_HEADER)base)->e_lfanew);
	PIMAGE_SECTION_HEADER section = IMAGE_FIRST_SECTION(ntHeaders);
	for (ULONG i = 0; i < ntHeaders->FileHeader.NumberOfSections; i++)
	{
		if (section[i].Characteristics & IMAGE_SCN_MEM_WRITE)
		{
			...
			if (DWORD(startOfSection) % sizeof(Page) != 0)
			{
				printf("WARNING: section %s is not page aligned\n", section[i].Name);
			}
			unsigned pagesToGuard = (section[i].Misc.VirtualSize + sizeof(Page) - 1) / sizeof(Page);
			unsigned pagesGuarded = 0;
#ifdef DEBUG_PRINT
			printf("Guarding %u pages\n", pagesToGuard);
#endif
			for (Page* page = (Page*)startOfSection;
				pagesGuarded < pagesToGuard;
				page++)
			{
#ifdef DEBUG_PRINT
				//printf("VA of guarded page: %p\n", page);
#endif
				DWORD oldProtect = 0;
				DWORD newProtect = 0;
...
#endif
				if (VirtualProtect(page, 1, newProtect, &oldProtect) == 0)
				{
					printf("Failed to VirtualProtect page at %p\n", page);
				}
				pagesGuarded++;
			}
#ifdef DEBUG_PRINT
			printf("Guarded %d pages (%d bytes)\n", pagesGuarded, pagesGuarded * sizeof(Page));
#endif
		}
	}
}

// Get the address of the copy of a specific page
Page* getCopyAddress(Page* addr)
{
	for (unsigned i = 0; i < sectionCopies.size(); i++)
	{
		DWORD minAddr = DWORD(sectionCopies[i].start);
		DWORD maxAddr = DWORD(sectionCopies[i].start + sectionCopies[i].size);
		DWORD a = DWORD(addr);
		DWORD offset = a - minAddr;
		if (a >= minAddr && a < maxAddr)
		{
			Page* returned = &(sectionCopies[i].ptr[offset / sizeof(Page)]);
			return returned;
		}
	}
	printf("Error: tried to get the copy of invalid address %p\n", addr);
	return nullptr;
}

void restoreMutableSections()
{
#ifdef USER_SEGHANDLE
	for (unsigned i = 0; i < modifiedPages.size(); i++)
	{
		//printf("Restoring page %p from %p\n", modifiedPages[i], getCopyAddress(modifiedPages[i]));
		memcpy(modifiedPages[i], getCopyAddress(modifiedPages[i]), sizeof(Page));
	}
#ifdef DEBUG_PRINT
	printf("Restored %d pages (%d bytes)\n", modifiedPages.size(), modifiedPages.size() * 4096);
#endif
	modifiedPages.clear();
#else
	for (unsigned i = 0; i < sectionCopies.size(); i++)
	{
#ifdef DEBUG_PRINT
		printf("Restoring section %s from backup (start = %p, ptr = %p, size = %d)\n", sectionCopies[i].name, sectionCopies[i].start, sectionCopies[i].ptr, sectionCopies[i].size);
#endif
		memcpy(sectionCopies[i].start, sectionCopies[i].ptr, sectionCopies[i].size);
	}
#endif
}
...
void InlineHook(PVOID pOrgFn, PVOID pNewFn, PVOID* ppOrgFnCall, size_t stolenCount)
{
	...
	*ppOrgFnCall = real_virtual_alloc_ptr(NULL, 0x100, MEM_COMMIT, PAGE_EXECUTE_READWRITE);
...
DWORD CALLBACK initThreadStart(LPVOID hModule)
{
	...
	setvbuf(fuzzer_stdout, NULL, _IONBF, 0);
	setvbuf(fuzzer_stdin, NULL, _IONBF, 0);
	SetConsoleTitleA("WinFuzz -- Server");
	...
	case DRYRUN:
		...
		fuzzer_printf("WinFuzz loaded - dryrun mode\n");
		break;
	case FORK:
		report_coverage = fork_report_coverage;
		report_crashed = fork_report_crashed;
		report_end = fork_report_end;
		fuzzer_printf("WinFuzz loaded - forkserver mode\n");
		break;
	case PERSISTENT:
		report_coverage = afl_report_coverage;
		report_crashed = afl_report_crashed;
		report_end = persistent_report_end;
		fuzzer_printf("WinFuzz loaded - persistent mode\n");
		break;
	default:
		FATAL("Invalid fuzzer mode");
	}
